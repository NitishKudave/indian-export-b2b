...
export default function Home() {
  ...
  // Fetch dynamic content on load if backend is available
  useEffect(() => {
    async function loadData() {
      try {
        const [catsData, prodsData, bannersData, testData] = await Promise.all([
          apiFetch("/categories/"),
          apiFetch("/products/"),
          apiFetch("/banners/"),
        ...
        if (prodsData && prodsData.length) setProducts(prodsData.slice(0, 4));
